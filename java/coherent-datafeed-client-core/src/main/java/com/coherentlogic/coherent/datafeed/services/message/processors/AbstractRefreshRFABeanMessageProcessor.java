...
public abstract class AbstractRefreshRFABeanMessageProcessor<T extends RFABean>
    extends AbstractRefreshOrUpdateEventRFABeanMessageProcessor<T> {

    public AbstractRefreshRFABeanMessageProcessor(RFABeanAdapter objectAdapter, Map ricCache, Map objectCache) {
        super(objectAdapter, ricCache, objectCache);
    }

    @Override
    protected void beforeUpdate(T target) {
        // target.clear (); // Required for refresh events since this is effectively resetting every variable in the
        // target object.
    }
