...
public class DispatchableNotificationClientImpl implements DispatchableNotificationClient {
     ...
     * @param timeout
     */
    public DispatchableNotificationClientImpl(EventQueue eventQueue, long timeout) {
    ...
    public void notify(Dispatchable dispatchable, Object closure) {
        ...
        log.debug("notify: method invoked; eventQueue: " + eventQueue + ", closure: " + closure);

        try {
            eventQueue.dispatch(timeout);
        } catch (DeactivatedException deactivatedException) {
            throw new EventQueueDispatchException(GENERIC_EXCEPTION_MSG, deactivatedException);
        } catch (DispatchQueueInGroupException dispatchQueueInGroupException) {
            throw new EventQueueDispatchException(GENERIC_EXCEPTION_MSG, dispatchQueueInGroupException);
        }
    }
}
